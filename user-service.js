  ...
  return new Promise(function (resolve, reject) {
    User.findById(id)
      ...
      .then((user) => {
        if (!user) {
          reject(
            `Unable to get favourites for user with id: ${id}. User not found.`
          );
          return;
        }
  ...
  return new Promise(function (resolve, reject) {
    User.findById(id)
      ...
      .then((user) => {
        if (!user) {
          reject(
            `Unable to add favourites for user with id: ${id}. User not found.`
          );
          return;
        }
        if (user.favourites.length < 50) {
          User.findByIdAndUpdate(
            ...
            .then((user) => {
              if (!user) {
                reject(
                  `Unable to update favourites for user with id: ${id}. User not found.`
                );
                return;
              }
              resolve(user.favourites);
          ...
          reject(
            `Unable to update favourites for user with id: ${id}.  Maximum favorites reached.`
          );
        }
      })
      .catch((err) => {
  ...
  return new Promise(function (resolve, reject) {
    User.findByIdAndUpdate(id, { $pull: { favourites: favId } }, { new: true })
      ...
      .then((user) => {
        if (!user) {
          reject(
            `Unable to remove favourites for user with id: ${id}. User not found.`
          );
          return;
        }
        resolve(user.favourites);
  ...
  return new Promise(function (resolve, reject) {
    User.findById(id)
      ...
      .then((user) => {
        if (!user) {
          reject(
            `Unable to get history for user with id: ${id}. User not found.`
          );
          return;
        }
